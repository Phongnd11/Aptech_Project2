...
public class Sys_FrameLoadProject extends JFrame {
	...
	public Sys_FrameLoadProject(int type, String id) {
	...
	protected void do_btnNewButton_actionPerformed(ActionEvent e) {
		...
		if(type==3) {
			...
			new BaoProject()
			.insert(new Project(txtId.getText(), txtAddNameProject.getText(), ((ComboItem) cbDepartment.getSelectedItem()).getId(),
					txtAddDescription.getText(),Integer.parseInt(txtAddPart.getText()), ((ComboItem) cbType.getSelectedItem()).getId(), date))
			.showMessage(this);
		}else {
			new BaoProject()
			.update(new Project(txtId.getText(), txtAddNameProject.getText(), ((ComboItem) cbDepartment.getSelectedItem()).getId(),
